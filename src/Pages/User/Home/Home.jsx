...
const Home = () => {
  ...
  // Get user from Redux store to determine login status
  ...
  const processAuthData = async (encodedData) => {
    ...
    try {
      ...
      // Dispatch user to Redux (adjust if your setUser expects full authData)
  ...
  // Moved above useEffect to fix "not defined" error
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white py-8">
        <div className="flex justify-center mb-8">
          <div className="bg-gray-100 rounded-full p-1 flex">
            ...
            <button
              ...
              className={`px-8 py-3 rounded-full font-semibold transition-all duration-300 ${activeTab === "jobs"
            ...
            {/* Doctors tab - only shown if logged in */}
            {isLoggedIn && (
              <button
                onClick={() => setActiveTab("doctors")}
                className={`px-8 py-3 rounded-full font-semibold transition-all duration-300 ${activeTab === "doctors"
                  ? "bg-bg-primary text-white shadow-lg"
                  : "text-gray-600 hover:text-gray-800"
                  }`}
              >
        ...
        <div className="min-h-[600px]">
          ...
          {activeTab === "doctors" && isLoggedIn && <DoctorsSection />}
          {activeTab === "doctors" && !isLoggedIn && (
            <div className="text-center py-20">
              <p className="text-xl text-gray-600">Please log in to view doctors.</p>
            </div>
          )}
        </div>
      </div>

      {/* Plans section - only visible to logged-in users */}
      {isLoggedIn && <Plans />}
